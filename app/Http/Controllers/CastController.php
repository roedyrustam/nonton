...
class CastController extends Controller
{
    ...
    public function showFilmographie($cast)
    {
        ...
        $movies = Movie::select('movies.title','movies.id','movies.poster_path','movies.vote_average','movies.updated_at')->where('active', '=', 1)
        ->addSelect(DB::raw("'movie' as type"))
        ->whereHas('casters', function($genre) use ($cast) {
            $genre->where('cast_id', '=', $cast);
        })->orderByDesc('id');


        $animes = Anime::select('animes.name','animes.id','animes.poster_path','animes.vote_average','animes.updated_at')->where('active', '=', 1)
        ->addSelect(DB::raw("'anime' as type"))->whereHas('casters', function($genre) use ($cast) {
            $genre->where('cast_id', '=', $cast);
        })->orderByDesc('id');

        $series = Serie::select('series.name','series.id','series.poster_path','series.vote_average','series.updated_at')->where('active', '=', 1)
